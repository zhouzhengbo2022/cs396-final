...
help("brewer.pal")
#high downloads as dependent variable
copurchase <- read.csv("copurchase.csv")
products <- read.csv("products.csv")

# Question 1
#filter products to only books that have a salesrank of <= 150000 and salesrank !=-1
attrs <- subset(products)
head(attrs)
book_id <- unique(attrs$id)
edges <- subset(copurchase, copurchase$Source %in% book_id & copurchase$Target %in% book_id)
head(edges)

download <- attrs[,6]
head(download)# use wave 2
download[download<2] <- 0 # set non-smoker to 0
...
actorGraph <- set_vertex_attr(actorGraph, "reviews_num", value = attrs$review_cnt)
actorGraph <- set_vertex_attr(actorGraph, "avg_rating", value = attrs$rating)
actorGraph <- set_vertex_attr(actorGraph, "high_rating", value = attrs$high_rating)
actorGraph <- set_vertex_attr(actorGraph, "downloads", value = attrs$downloads)
...
head(products)
data$vertices$avg_rating
res.0 <- BayesALAAM(y = data$vertices$downloads,           # dependent variable
                    ADJ = adj,           # network
